...
class VoiceConversionWrapper(torch.nn.Module):
    ...
    def convert_voice_with_streaming(
        ...
        processed_frames = 0
        total_frames = source_mel_len
        last_progress = -1
        progress_step = 5  # only log when percentage increases by this step
        previous_chunk = None
        if convert_style:
            with torch.autocast(device_type=device.type, dtype=dtype):
            ...
            for i in range(0, len(src_narrow_reduced), max_chunk_size):
                ...
                with torch.autocast(device_type=device.type, dtype=dtype):
                    ...
                    vc_mel = self.cfm.inference(
                        ...
                        target_mel, target_style, diffusion_steps,
                        inference_cfg_rate=[intelligibility_cfg_rate, similarity_cfg_rate],
                ...
                # Progress logging for visibility in server logs (throttled)
                progress = min(99, int(100 * min(processed_frames, total_frames) / max(1, total_frames)))
                if progress >= last_progress + progress_step:
                    print(f"[progress] v2 conversion {progress}%")
        ...
        if last_progress < 100:
            print("[progress] v2 conversion 100%")
