...
# Melody post-processing (singing-ish) imports
try:
    from ..audio.melody_post import make_more_singing
    MELODY_POST_AVAILABLE = True
except Exception as e:
    logging.warning(f"⚠️  Melody post-processing not available: {e}")
    MELODY_POST_AVAILABLE = False

# Diff-SVC (singing voice conversion) imports
try:
    from ..audio.diffsvc_converter import DiffSVCConverter
    from ..config import DIFFSVC_DEFAULT_MODEL
    DIFFSVC_AVAILABLE = True
    diffsvc_converter = DiffSVCConverter(model_path=DIFFSVC_DEFAULT_MODEL)
    logging.info("✅ Diff-SVC scaffold available")
except Exception as e:
    logging.warning(f"⚠️  Diff-SVC not available: {e}")
    DIFFSVC_AVAILABLE = False
    diffsvc_converter = None
...
class VoiceCloneRequest(BaseModel):
    ...
    singing_mode: bool = False  # Apply melody-style post-processing
...
class DiffSVCRequest(BaseModel):
    source_audio_id: str
...
async def clone_voice(request: VoiceCloneRequest):
    ...
    try:
        ...
        # Optional singing-style post-processing
        if request.singing_mode and MELODY_POST_AVAILABLE:
            logging.info("Applying singing-style post-processing (melody/vibrato heuristic)...")
            try:
                cloned_audio = make_more_singing(cloned_audio, sample_rate)
                logging.info("✅ Singing post-process applied")
            except Exception as e:
                logging.warning(f"Singing post-process failed, using original audio: {e}")
...
@app.post("/api/diffsvc_convert")
async def diffsvc_convert(request: DiffSVCRequest):
    """Convert uploaded singing/rap audio using Diff-SVC (placeholder)."""
    if not DIFFSVC_AVAILABLE or not diffsvc_converter:
        raise HTTPException(status_code=503, detail="Diff-SVC not available")

    if request.source_audio_id not in audio_storage:
        raise HTTPException(status_code=400, detail="Invalid source audio ID")

    try:
        source_info = audio_storage[request.source_audio_id]
        source_path = Path(source_info["file_path"])

        if not diffsvc_converter.is_ready():
            logging.warning("Diff-SVC model path not set; returning pass-through audio")

        output_path = Path(tempfile.mktemp(suffix="_diffsvc.wav"))
        converted_path, sample_rate = diffsvc_converter.convert(
        ...
        return FileResponse(
            converted_path,
            media_type="audio/wav",
            filename=f"diffsvc_{int(time.time())}.wav"
        )

    except Exception as e:
        logging.error(f"Diff-SVC conversion failed: {e}")
        raise HTTPException(status_code=500, detail=f"Diff-SVC failed: {str(e)}")
